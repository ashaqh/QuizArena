      ...
      try {
        ...
        if (response != null && response.isNotEmpty) {
          ...
          if (questions.isNotEmpty && _validateQuestions(questions, topic)) {
            debugPrint(
              '✅ ${provider.name} succeeded with ${questions.length} questions',
            );
            return questions;
          } else {
...
''';
            ...
            'messages': [
              {
                'role': 'system',
                'content':
                    'You are an expert quiz creator. Generate only well-formatted multiple choice questions in the exact format requested.',
              },
  ...
  Question? _parseSingleQuestionImproved(String block) {
    try {
      ...
      if (correctLetter == null ||
          !['A', 'B', 'C', 'D'].contains(correctLetter)) {
  ...
  bool _validateQuestions(List<Question> questions, String topic) {
    ...
    for (final question in questions) {
      ...
      if (question.answers.length != 4) {
        debugPrint(
          '❌ Question has wrong number of answers: ${question.answers.length}',
        );
        return false;
      }
      ...
      if (correctAnswers != 1) {
        debugPrint(
          '❌ Question has $correctAnswers correct answers (should be 1)',
        );
        return false;
      }
    }
    ...
    for (int i = 0; i < count && i < fallbackTemplates.length; i++) {
      ...
      final question = Question(
        ...
        answers: [
          Answer(
            id: correctAnswerId,
            text: template['correct'] as String,
            isCorrect: true,
          ),
          Answer(
            id: _uuid.v4(),
            text: template['wrong1'] as String,
            isCorrect: false,
          ),
          Answer(
            id: _uuid.v4(),
            text: template['wrong2'] as String,
            isCorrect: false,
          ),
          Answer(
            id: _uuid.v4(),
            text: template['wrong3'] as String,
            isCorrect: false,
          ),
        ],
  ...
  List<Map<String, String>> _getTopicSpecificTemplates(
    String topic,
    String difficulty,
  ) {
    final topicLower = topic.toLowerCase();
  ...
  List<Question> _createGenericFallbacks(
    String topic,
    String difficulty,
    int count,
  ) {
    ...
    for (int i = 0; i < count && i < 5; i++) {
      ...
      final question = Question(
        id: _uuid.v4(),
        text:
            'Question $questionNumber: What is an important aspect of $topic? (${difficulty.toUpperCase()} level - Please edit this question)',
        answers: [
          Answer(
            id: correctAnswerId,
            text: 'Key concept in $topic (CORRECT - Please edit)',
            isCorrect: true,
          ),
          Answer(
            id: _uuid.v4(),
            text: 'Alternative option A (Please edit)',
            isCorrect: false,
          ),
          Answer(
            id: _uuid.v4(),
            text: 'Alternative option B (Please edit)',
            isCorrect: false,
          ),
          Answer(
            id: _uuid.v4(),
            text: 'Alternative option C (Please edit)',
            isCorrect: false,
          ),
        ],
        correctAnswerId: correctAnswerId,
  ...
  Future<List<Map<String, dynamic>>> getAvailableModels() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '🤖 ImprovedAI: Failed to fetch models: ${response.statusCode}',
        );
        return [];
      }
    } catch (e) {
