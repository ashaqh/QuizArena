...
class _GameResultsScreenState extends ConsumerState<GameResultsScreen> {
  ...
  void initState() {
    super.initState();
    // Delay the save operation to avoid modifying providers during widget build
    Future(() => _saveGameData());
  }
      ...
      if (user != null) {
        ...
        if (userPlayer == null) {
          ...
          if (nonHostPlayers.isNotEmpty) {
            ...
            userPlayer = nonHostPlayers.first;
            debugPrint('GameResults: Using fallback player: ${userPlayer.id}');
          }
        }

        if (userPlayer != null) {
          // Update the currentPlayerProvider with the final scores
          ref.read(currentPlayerProvider.notifier).state = userPlayer;
        ...
        debugPrint('GameResults: User not authenticated');
      }
    } catch (e) {
      debugPrint('GameResults: Error saving game data: $e');
